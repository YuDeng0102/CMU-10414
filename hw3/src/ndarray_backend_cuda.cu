...
__device__ size_t get_idx(size_t idx,CudaVec shapes,CudaVec strides,size_t offset){
  size_t res=offset;
  size_t sz=1;
  for(int i=shapes.size-1;i>=0;i--){
    auto val=idx%(shapes.data[i]*sz)/sz;
    res+=val*strides.data[i];
    sz*=shapes.data[i];
  }
  return res;
}
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  if(gid<size) {
    out[gid]=a[get_idx(gid,shape,strides,offset)];
  }
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if(gid<size) {
    out[get_idx(gid,shape,strides,offset)]=a[gid];
  }
}
...
__global__ void ScalarSetitemKernel(size_t size, scalar_t val, scalar_t* out, CudaVec shape,
  ...
  /// BEGIN SOLUTION
  if (gid < size) {
    out[get_idx(gid, shape, strides, offset)] = val;
  }
  /// END SOLUTION
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t reduce_size, size_t size) {
  ...
  if (i < size) {
    ...
    for (size_t j = 0; j < reduce_size; j++) {
      reduce_max = max(reduce_max, a[offset + j]);
    }
    out[i] = reduce_max;
  }
}
