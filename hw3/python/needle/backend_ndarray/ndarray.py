...
class NDArray:
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        assert all( new_shape[i]==self.shape[i] or self.shape[i]==1 
                   for i in range(len(self.shape))),"shape don't match!"
        new_strides=tuple([self.strides[i] if new_shape[i]==self.shape[i] else 0 for i in range(len(self.shape))])
    ...
    def __getitem__(self, idxs):
        ...
        return self.make(shape,strides,self.device,self._handle,offset)
        ### END YOUR SOLUTION
