...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    # instead of the vector output_ndode. But this is the common case for loss function.
    ...
    for node in reverse_topo_order:
        ...
        if len(node.inputs)>0:
            son_grads=node.op.gradient_as_tuple(node.grad,node)
            for i,son in enumerate(node.inputs):
                if son not in node_to_output_grads_list:
                    node_to_output_grads_list[son]=[]
                node_to_output_grads_list[son].append(son_grads[i])
    ### END YOUR SOLUTION
...
def topo_sort_dfs(node, visited:set, topo_order:list):
    """Post-order DFS"""
    ### BEGIN YOUR SOLUTION
