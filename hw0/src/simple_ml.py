...
def softmax_loss(Z, y):
    ...
    """
    ### BEGIN YOUR CODE
    return (np.log(np.sum(np.exp(Z),axis=1))-Z[np.arange(Z.shape[0]), y]).mean()
    ### END YOUR CODE
...
def softmax_regression_epoch(X, y, theta, lr = 0.1, batch=100):
    ...
    """
    ### BEGIN YOUR CODE
    tot_batchs=X.shape[0]//batch
    for i in range(tot_batchs):
        X_b=X[i*batch:(i+1)*batch]
        y_b=y[i*batch:(i+1)*batch]
        one_hot=np.zeros((batch,theta.shape[1]))
        one_hot[np.arange(batch),y_b]=1
        Z=X_b@theta
        Z=np.exp(Z)
        Z/=np.sum(Z,axis=1,keepdims=True)
        theta-=lr*X_b.T @ (Z-one_hot)/batch
...
def nn_epoch(X, y, W1, W2, lr = 0.1, batch=100):
    ...
    for i in range(tot_batchs):
        X_b=X[i*batch:(i+1)*batch]
        y_b=y[i*batch:(i+1)*batch]
        one_hot=np.zeros((batch,W2.shape[1]))
        one_hot[np.arange(batch),y_b]=1
        Z1=np.maximum(X_b@W1,0)
        G2=np.exp(Z1@W2)
        G2=G2/np.sum(G2,axis=1,keepdims=True)-one_hot
        G1=(Z1>0)*(G2 @ W2.T)
        W1-=lr*X_b.T@G1/batch
        W2-=lr*Z1.T@G2/batch
